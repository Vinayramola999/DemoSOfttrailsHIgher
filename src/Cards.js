...
const ProfileDashboard = () => {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        await axios.post(`${MAIN_API_BASE}/users/verify-token`,
          { token }
        );
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!userId) return;
      try {
        const response = await axios.get(`${MAIN_API_BASE}/users/id_user/${userId}`,{headers: { Authorization: `Bearer ${token}` },});
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-custome-blue rounded-lg w-full px-3 py-3 flex flex-col sm:flex-row justify-between items-center shadow-lg sticky top-0 z-10">
        <h1 className="bg-white text-2xl sm:text-3xl font-bold flex items-center">
