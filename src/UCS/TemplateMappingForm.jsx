...
const TemplateMappingForm = ({ activeTab, allModules, subTabs, moduleTableNames }) => {
  ...
  const getAllSms = async () => {
    try {
      ...
      tableDataRes.data.forEach((item) => {
        item?.mappedVariables?.forEach((variable) => {
          if (
            (notificationType === "email" && variable.type === "Email") ||
            (notificationType === "sms" && variable.type === "SMS")
          ) {
            // ✅ Service name pick karo type ke hisaab se
            let serviceName =
              variable.type === "Email"
                ? item.emailService || "Not mapped"
                : variable.type === "SMS"
                  ? item.smsService || "Not mapped"
                  : "Not mapped";
            ...
            finalData.push({
              ...
              moduleName: variable.moduleName || item.moduleName,
              notificationType: variable.type,
              variables: variable.variables || "N/A",
              mappedVariables:
                variable.mappedVariables ||
                variable.othersMappedVariables ||
                "N/A",
            });
          }
        });
      ...
      finalData.forEach(item => {
        ...
        result[item.serviceName] = {
          ...result[item.serviceName],
          variables: [
            ...result[item.serviceName].variables,
            ...(Array.isArray(item.variables) ? item.variables : [item.variables])
          ],
          mappedVariables: [
            ...result[item.serviceName].mappedVariables,
            ...(Array.isArray(item.mappedVariables) ? item.mappedVariables : [item.mappedVariables])
          ]
        };
      });


      console.warn('final:', Object.entries(result));

      setTableData(Object.entries(result));
