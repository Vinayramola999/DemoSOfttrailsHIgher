...
//       if (!hasAmsAccess) {
//         try {
//           const response = await axios.get(`${MAIN_API_BASE}/access`,
//             {
//               headers: {
...
//         } catch (error) {
//           console.error(
//             "Error updating API access for update_access:",
//             error.response ? error.response.data : error.message
//           );
//           Swal.fire({
//             icon: "error",
//             title: "Error",
//             text: "Error updating API access for update_access.",
//           });
//         }
//       }
//       return; 
...
//         // Update permission map if applicable
//         if (permissionMap[apiName]) {
//           permissionMap[apiName]((prev) => !prev);
//         }
//       } catch (error) {
//         console.error(
//           `Error updating API access for ${apiName}:`,
//           error.response ? error.response.data : error.message
//         );
//         Swal.fire({
//           icon: "error",
//           title: "Error",
//           text: `Error updating API access for ${apiName}.`,
//         });
//       }
//     }
//   };
...
//     const selectedModule = "AMS";
//     const selectedApiAccess = [];
//     if (hasAmsAccess) selectedApiAccess.push("update_access");
...
//                   checked={apiAccess.includes("update_access")}
                    ...
                    <div className="flex flex-col sm:flex-row mt-6">
                        <div className="w-full sm:w-1/2  rounded-lg p-4 h-auto max-h-[450px] overflow-y-auto">
                            <div className=" rounded-lg overflow-y-auto">
                                <label className="flex items-center">
                                    <input type="checkbox" onChange={() => handleApiAccessChange("update_access")} checked={hasAmsAccess} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                                    <span className="text-blue-600 text-lg font-bold ml-5"> Access Privilege </span>
                                </label>
                            </div>
                        </div>
                    </div>
