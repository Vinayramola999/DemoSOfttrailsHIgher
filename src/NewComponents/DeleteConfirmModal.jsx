...
import { DeleteIcon } from "./ReactIcons";
  ...
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white w-[360px] p-10 rounded-xl shadow-xl text-center space-y-4">
        ...
        <div className="flex justify-center gap-4 pt-2">
          <button onClick={onCancel} className="border border-gray-300 px-10 py-2 rounded-md text-sm hover:bg-gray-100" disabled={loading} > Cancel </button>
          <button onClick={onConfirm} className="bg-red-600 text-white px-10 py-2 rounded-md text-sm hover:bg-red-700 transition" disabled={loading} > {loading ? "Deleting..." : "Delete"} </button>
        </div>
      </div>
    </div>
  );
